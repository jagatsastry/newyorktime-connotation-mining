...
cat all_months | xargs -P 16 -I % python neighbor_vote_multiplier_conn.py % 
mkdir graphs/neighbor_vote_multiplier_reports.conn

function generate_month_rep ()
{

    month=$1
    fl=graphs/neighbor_vote_multiplier_reports.conn/"$month".report
    iter=0
    printf $month
